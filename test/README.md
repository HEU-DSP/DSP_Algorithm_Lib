...
仓库目录已进行 breaking include-path 迁移，旧工程使用的 `FFTNt.h`、`ffttest.h`、`IIR.h` 等路径需要按根目录 README 的迁移表更新。
...
- 测幅：正弦波、方波、三角波；
- 测相：`-170°、-90°、-30°、0°、45°、90°、170°`、带噪 45°，以及非相干频率/带噪非相干频率；
- CZT：两组非整数频率和不同幅值。
- 测相配套测幅：正弦、方波、三角波幅度，以及保护和非法输入检查；
- FIR：对给定 11 个归一化系数的确定性正弦与冲激 NumPy 卷积输出比较，并覆盖零输入安全；
...
完整运行会在 JSON 的 `resources` 中保存 16 条算法 BENCH 平均耗时及 GNU `size -B` 的可执行文件、`.text`、`.data`、`.bss` 大小，覆盖频率、FFT 核心、RMS 幅值、测相配套幅值、I/Q 测相、CZT、FIR 和安全包装器目标。它们都是 PC/GCC 主机参考值：主机时间会随机器和负载变化，不能复制为 STM32 性能；同一测试目标中的算法共享同一个目标映像大小，因此同目标的大小行不是单个函数的独立大小。`test_sanity` 只验证 CMake 配置、CMSIS 链接和工具链可用性，不属于算法运行时资源目标。
